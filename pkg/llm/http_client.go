...
//ff:what llmClient — pkg/llm 세 어댑터(ollama/gemini/openai-compat)가 공유하는 단일 http.Client. 300s 타임아웃으로 무인 loop 실행에서 호출 1건이 행걸려 루프 전체가 영구 정지하는 것을 막는다(http.DefaultClient/http.Post 무타임아웃 대체).
...
// llmTimeout bounds every adapter HTTP call so an unattended loop run can never
