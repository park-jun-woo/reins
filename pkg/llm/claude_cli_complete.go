...
//ff:what ClaudeCLI.Complete — `claude -p` argv를 빌드(plain claude -p, --bare 미사용)하고 execClaude seam으로 돌려 단일턴 생성을 수행한다. --output-format json·--append-system-prompt system(전체 교체 아님)·--max-turns(MaxTurns||1, 도구 루프 차단)·--permission-mode dontAsk; Model 있으면 --model, Stateless→--no-session-persistence·Continue→sid 있으면 --resume sid. user는 stdin(긴 본문 arg-length 회피), ctx는 llmTimeout 300s. JSON(result/session_id/is_error/subtype) 파싱: 비정상 종료(stderr 동봉)·파싱 실패·is_error 각각 에러; Continue면 sid 갱신; result 반환.
