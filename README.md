...
| `pkg/cli` | Cobra 스캐폴드 — `NewQuestCmd` → scan/next/submit/status/export/rules (+ 옵트인 `agent`) | cobra, quest, gate, llm |
...
agent   (옵트인) submit의 자동 반복 — LLM이 생성, 게이트가 판정·잠금 (아래)
```

## 무인 자동 구동 — `agent` 명령 (옵트인)

외부 에이전트가 손으로 `next`→`submit` 하던 흐름을, **CLI 안에서 LLM이 생성하고 게이트가 판정하는 루프**로 닫는다. `cli.Options{Agent: &cli.AgentOptions{…}}`로 옵트인하면 `agent` 명령이 붙는다(미설정이면 미부착·완전 후방호환).
...
ccnews agent --model ollama:gemma4:e4b   # 남은 TODO를 gemma4가 생성→게이트 판정
```
...
커버), gofmt clean. 레벨집계 + defeat 그래프 백엔드(toulmin) + ground/staged 평가 + **`agent` 무인
...
- `plans/reins/` — 설계문서 (Phase001~006 v1 스캐폴드, Phase007 toulmin 그래프 백엔드, Phase008 payload rehydration, Phase009 agent 명령·`pkg/llm`)
